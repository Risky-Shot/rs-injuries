...
CreateThread(function()
	while true do
        ...
        temperature = math.floor(GetTemperatureAtCoords(pedCoords) + 0.5)

        boneSpeedLimit = LocalPlayer.state.boneSpeedLimit

        if LocalPlayer.state.temperature ~= temperature then
            LocalPlayer.state.temperature = temperature
        end
    ...
    while true do
            ...
            if isRagdollThisTick then
                ...
                if hasDamagedBone and ragdollLastDamagedBone ~= damagedBoneId then
                    ...
                    if (fallHeight > 4.0) then
                        ...
                        healthManager.triggerFallDamage(fallHeight, damagedBoneId)
                        ragdollLastZ = coords.z
                    end
                end
        ...
        if isOnFoot then
            ...
            if currentMoveBlend >= 2.0 or (isCarryingSomething and currentMoveBlend >= 1.0) then
                ...
                stamina = clamp(stamina - lerp(0.0, lerpTo, currentMoveBlend / 3.0), 0, 100)
            ...
            elseif stamina < 100 and not isRagdollThisTick then
                ...
                stamina = clamp(stamina + lerp(lerpTo, 0.0, currentMoveBlend / 1.5), 0, 100)
                -- InvokeNative(0xC6258F41D86676E0, playerPed, 1, stamina) -- _SET_ATTRIBUTE_CORE_VALUE
            end
